...
## jvm进程和jvm的关系
> 一个进程对应于一个程序的执行，进程是动态的概念，而程序为静态的概念。多个进程可以并发执行同一个程序
...
### 2）非标准参数（-X）
默认 jvm 实现这些参数的功能，但是并不保证所有jvm实现都满足，不保证向后兼容
...
### 3）非稳定参数（-XX）
非稳定（Stable）参数，此类参数各个 jvm 实现会有所不同，将来可能会随时取消，需要慎重使用

输入命令 java -XX:+PrintFlagsFinal 即可查看
...
ccstrlist CompileOnly                               =                                     {product}
   ...
   double G1ConcMarkStepDurationMillis              = 10.000000                           {product}
    ...
    uintx MaxNewSize                               := 1422393344                          {product}
